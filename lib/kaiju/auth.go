...
import (
	...
	exported "github.com/tessornetwork/kaiju/lib/legacydid"
	"github.com/spf13/pflag"
)

var (
	expectedMinGasPrices       = "0.025" + KaijuNativeToken
	approximationGasAdjustment = float64(1.5)
	fees                       = "1000000" + KaijuNativeToken
...
type PubKeyGetter func(ctx sdk.Context, msg KaijuMsg, sigs []signing.SignatureV2) (cryptotypes.PubKey, error)
...
// 	// need a custom kaiju AnteHandler. Below, we will discuss the differences
// 	// between the default Cosmos AnteHandler and our custom kaiju AnteHandler.
// 	//
...
// 	//
// 	// In general:
// 	// - Enforces messages to be of type KaijuMsg, to be used with pubKeyGetter.
...
func GenerateOrBroadcastTxCLI(clientCtx client.Context, flagSet *pflag.FlagSet, kaijuDid exported.KaijuDid, msg sdk.Msg) error {
	txf := tx.NewFactoryCLI(clientCtx, flagSet)
	return GenerateOrBroadcastTxWithFactory(clientCtx, txf, kaijuDid, msg)
}

func GenerateOrBroadcastTxWithFactory(clientCtx client.Context, txf tx.Factory, kaijuDid exported.KaijuDid, msg sdk.Msg) error {
	if clientCtx.GenerateOnly {
		return tx.GenerateTx(clientCtx, txf, msg) // like old PrintUnsignedStdTx
	}

	return BroadcastTx(clientCtx, txf, kaijuDid, msg)
}

func BroadcastTx(clientCtx client.Context, txf tx.Factory, kaijuDid exported.KaijuDid, msg sdk.Msg) error {
	...
	err = Sign(txf, clientCtx, tx, true, kaijuDid)
	if err != nil {
		return err
	}
...
func Sign(txf tx.Factory, clientCtx client.Context, txBuilder client.TxBuilder, overwriteSig bool, kaijuDid exported.KaijuDid) error {
	var privateKey ed25519.PrivKey
	privateKey.Key = append(base58.Decode(kaijuDid.Secret.SignKey), base58.Decode(kaijuDid.VerifyKey)...)
...
func KaijuSigVerificationGasConsumer(
...
func SignAndBroadcastTxFromStdSignMsg(clientCtx client.Context,
	msg sdk.Msg, kaijuDid exported.KaijuDid, flagSet *pflag.FlagSet) (*sdk.TxResponse, error) {
